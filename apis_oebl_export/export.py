...
def export_persons():
    ...
    while nextpage:
        ...
        for result in data["results"]:
            ...
            result["texts"] = [text for key, text in texts.items() if key in textids]
...
def export_texts():
    ...
    outputfolder = pathlib.Path("texts")
    ...
    while nextpage:
        ...
        for result in data['results']:
            ...
            if result["id"] not in [2, 236, 5811]:
                ...
                print(result['url'])
    pathlib.Path("texts.json").write_text(json.dumps(texts, indent=2))
